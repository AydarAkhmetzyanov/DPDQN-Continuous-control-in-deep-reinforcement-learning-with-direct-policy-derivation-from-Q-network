...
class DPDQN2:
    ...
    def evaluate(self, n_games=1, greedy=True, t_max=10000, num_action_samples=50):
        rewards = []
        for _ in range(n_games):
            s = self.env.reset()
            reward = 0
            for _ in range(t_max):
                action = self.predict(s, greedy=greedy, num_action_samples=num_action_samples)[0]
    ...
    def learn(self, total_timesteps=1e5):
        ...
        if self.verbose==0 and self.progress_bar == False:
            trange = range
        else:
            from tqdm import trange
        ...
        for step in trange(total_timesteps-self.warm_up):
            ...
            #skip for verbose
            if (self.ray_tune or self.verbose) and step % eval_freq == 0:
    ...
    def predict(self, obs, greedy=False, num_action_samples=10):
        if greedy and num_action_samples<100:
